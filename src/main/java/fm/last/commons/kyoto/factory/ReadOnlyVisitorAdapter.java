...
class ReadOnlyVisitorAdapter implements Visitor {

  private final ReadOnlyVisitor delegate;

  ReadOnlyVisitorAdapter(ReadOnlyVisitor delegate) {
    this.delegate = delegate;
  }

  @Override
  public byte[] visit_empty(byte[] key) {
    delegate.emptyRecord(key);
    return Visitor.NOP;
  }

  @Override
  public byte[] visit_full(byte[] key, byte[] value) {
    delegate.record(key, value);
    return Visitor.NOP;
  }
